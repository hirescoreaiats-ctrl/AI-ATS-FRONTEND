(() => {
  ...
  function partialMatchKey(a, b) {
    ...
    if (a && b && typeof a === "object" && typeof b === "object") {
      ...
      for (const key2 of bKeys) {
        if (!partialMatchKey(a[key2], b[key2])) {
          return false;
        }
      }
      return true;
  ...
  var key = () => ["dashboard", "summary", identity()];
  var storageKey = () => PREFIX + identity();
  ...
  window.addEventListener("focus", () => window.loadDashboard?.(true));
  window.addEventListener("online", () => window.loadDashboard?.(true));
