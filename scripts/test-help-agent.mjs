...
assert.match(html, /help-agent\.css\?v=recruiting-agent-20260823-12/, "agent css should be loaded");
assert.match(html, /help-agent\.js\?v=recruiting-agent-20260823-12/, "agent js should be loaded");
...
assert.match(help, /Can you share your requirements or JD\?/, "Action Agent should request requirements or JD before creating a job");
assert.match(help, /shouldStartConversationalJobCreation/, "create-job execution requests should bypass page-navigation matching");
assert.match(help, /ACTION_ROUTED_FEATURE_WORKFLOWS/, "all supported Action Agent feature requests should bypass page navigation when execution was requested");
assert.match(help, /shouldKeepRequestInsideActionAgent/, "execution and pure navigation requests should be routed separately across supported workflows");
assert.match(help, /function\s+isNavigationRequest[\s\S]*?\\b\(\?:open\|show\|view/, "navigation detection should use whole words instead of matching view inside interview");
assert.match(help, /isExplicitNavigationCommand\(value\)[\s\S]*?forceNavigation:true/, "explicit open/show commands should route before action planning");
assert.match(help, /await handleWorkflow\(\{intent:feature\.workflowId[\s\S]*?return true;/, "product routing should stop after delegating to a contextual workflow");
assert.match(help, /candidateActionLanguage/, "candidate action phrases should never be misrouted as talent discovery queries");
assert.match(help, /currentJobFormDraft/, "Action Agent should preserve create-job form context already entered on the page");
assert.match(help, /Creating a new job/, "create-job page should expose an explicit context label inside the agent");
assert.match(help, /Already captured from this page/, "job creation prompt should summarize existing form context compactly");
...
assert.match(help, /hasOwnProperty\.call\(next,"candidate_id"\)/, "clearing a candidate should also normalize stale candidate context");
